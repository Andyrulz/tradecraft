      ...
      <Card className="border-2 border-primary/20 shadow-lg w-full">
        <CardHeader>
          <div className="flex items-center gap-2">
            <CardTitle className="text-lg sm:text-xl">Stock Scorecard</CardTitle>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {Object.entries(analysis.scorecard).map(([key, value]) => (
              <div key={key} className="p-3 sm:p-4 rounded-lg border bg-card shadow-sm hover:shadow-md transition-shadow w-full">
                <div className="flex items-center gap-2 mb-2 sm:mb-3">
                  {getScoreIcon(key)}
                  <h3 className="text-sm sm:text-base font-semibold capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</h3>
                </div>
