...
export async function POST() {
  try {
    ...
    for (const { symbol, company_name } of stocks) {
      ...
      console.log(`[Screener POST] Processed ${symbol} at ${new Date().toISOString()}`);
      // Add delay to keep API calls under 100/minute
    ...
    if (results.length === 0) {
      console.error('No results generated for top stocks.');
    }
